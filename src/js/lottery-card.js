...
(function() {
        ...
        card.innerHTML = `
            <div style="display:flex;justify-content:space-between;align-items:center;">
                <div style="display:flex;align-items:center;gap:8px;">
                    ...
                    <h4 style="margin:0;">Customization Lottery Card</h4>
                </div>
                <button id="lottery-close-btn" title="Close">×</button>
            </div>
            <div class="form-group">
                <label>Lottery Name</label>
                <input type="text" id="lottery-title" placeholder="Ex, Annual Lottery, Lucky Wheel" style="width:100%;padding:6px 10px;font-size:15px;border-radius:5px;border:1px solid #ccc;">
            </div>
            <div class="form-group" style="position:relative;">
                <label>Prize Items (One per line)</label>
                <textarea id="lottery-items" placeholder="Ex, First Prize\nSecond Prize\nThird Prize" style="padding:8px 10px;width:100%;height:80px;font-size:15px;border-radius:5px;border:1px solid #ccc;"></textarea>
                <button id="lottery-clear-btn" style="position:absolute;right:6px;top:28px;background:#e53935;color:#fff;border:none;border-radius:5px;padding:2px 10px;font-size:13px;cursor:pointer;">Clear</button>
            </div>
            <div class="form-group">
                <label>Number of Prizes to Draw</label>
                <input type="number" id="lottery-count" min="1" max="20" value="1">
            </div>
            <div class="actions">
                <button id="lottery-draw-btn">Start Lottery</button>
                <button id="lottery-share-btn" style="background:#4caf50;display:none;">Share Results</button>
            </div>
    ...
    function showLotteryCard() {
        ...
        panel.querySelector('#lottery-draw-btn').onclick = function() {
            ...
            const title = panel.querySelector('#lottery-title').value.trim() || 'Customization抽奖';
            if (!itemsText) {
                panel.querySelector('#lottery-result').textContent = '请填写奖项Content';
            ...
            // 展示统计
            let statsHtml = '<b>抽奖统计：</b><br>' + items.map(it => `${it}：${lotteryStats[it]||0} times`).join(' &nbsp; ');
            ...
            spinWheel(canvas, items, showIndex, function(){
                panel.querySelector('#lottery-result').innerHTML = `<span style='font-size:22px;'>🎉</span> <b>${title}</b> 抽medium：<br>${result.map(r => `<span style='display:inline-block;margin:4px 0;'>${r}</span>`).join('<br>')}`;
                shareBtn.style.display = '';
                // 记录本times结果，供Share
                const emojiTitle = `🎲【${title}】幸运抽奖`;
                const emojiResult = `🎉 抽medium：${result.join('，')} ✨`;
                const emojiStats = `📊 全部统计：\n${items.map(it => `🍀${it}：${lotteryStats[it]||0}times`).join('，')}`;
        ...
        panel.querySelector('#lottery-share-btn').onclick = function() {
            ...
            if (navigator.clipboard) {
                navigator.clipboard.writeText(text).then(() => {
                    ...
                    setTimeout(()=>{this.textContent='Share结果';}, 1200);
                });
            } else {
                // 兼容性降级
