...
const UIManager = {
    ...
    preventPageShift() {
        ...
        document.addEventListener('touchmove', (e) => {
            // 如果触摸Things发生在可滚动元素内，则允许滚动
            if (e.target.closest('.task-List, .view-content, .modal-content, .settings-content, .countdown-List, .Lists-nav, .List-items-container, .app-container, main, section, .projects-container, .view-header, .create-tabs, .form-group, .modal-body, .import-container, .import-section, .ai-modal-content, .kimi-iframe, .analog-clock, .focus-timer, .timer-display, .timer-controls, .focus-container, .focus-stats, .countdown-container, .relax-container, .wheel-container, .List-content, .date-header, .weather-tips-container, .modal, button, input, select, textarea')) {
                return;
            }
            
            // 仅在视口顶部和底部进行弹性滚动时阻止默认行为
            if (document.documentElement.scrollTop <= 0 && e.touches[0].clientY > 0) {
                // 防止顶部下拉刷新
                e.preventDefault();
            } else if ((document.documentElement.scrollTop + window.innerHeight) >= document.documentElement.scrollHeight && e.touches[0].clientY < window.innerHeight) {
                // 防止底部上拉刷新
                e.preventDefault();
            }
        }, { passive: false });
    ...
    createTVModeToggle() {
        ...
        tvModeToggle.title = '切换-适合远距离观看的界面';
    ...
    disableTVMode() {
        ...
        // 显示通知
        this.showNotification('已Close电视模式');
    },
    ...
    disableRelaxButton(disable) {
        ...
        if (disable) {
            ...
            relaxNavBtn.title = '计时进行medium，Downtime不可用';
    ...
    bindEvents() {
        ...
        if (this.elements.settingsAvatar) {
            this.elements.settingsAvatar.addEventListener('change', (e) => {
                const file = e.target.files[0];
                if (file) {
                    // 验证文件类型
                    if (!file.type.startsWith('image/')) {
                        this.showNotification('请Select图片文件');
                        return;
                    }
    ...
    validateNickname(nickname) {
        ...
        // 检查特殊字符（只允许medium文、英文、数字和下划线）
        if (!/^[\u4e00-\u9fa5a-zA-Z0-9_]+$/.test(nickname)) {
            result.isValid = false;
            result.message = '昵称只能包含medium文、英文、数字和下划线';
            return result;
        }
        ...
        if (settingsAvatar) {
            ...
            settingsAvatar.addEventListener('change', function(e) {
                ...
                if (!file.type.match('image.*')) {
                    UIManager.showNotification('请Select图片文件（JPG、PNG格式）', 'error');
                    return;
                }
    ...
    autoSaveSettings(settingType) {
        ...
        switch (settingType) {
            ...
            case 'display':
                ...
                this.showNotification(updatedSettings.tvMode ? '已开启电视模式' : '已Close电视模式');
                break;
            ...
            case 'notifications':
                ...
                this.showNotification('通知Set已更新');
                break;
            ...
            case 'focus':
                ...
                this.showNotification('FocusSet已更新');
                break;
            ...
            case 'ai':
                ...
                this.showNotification('AISet已更新');
                break;
    ...
    handleLogin() {
        ...
        if (dataRestored) {
            ...
            this.showNotification(`Welcome，${nickname}！`);
        }
        // 登录成功后自动刷新页面，彻底解决DOM和Things绑定问题
        setTimeout(() => {
        ...
        input.onchange = (e) => {
            ...
            reader.onload = (e) => {
                try {
                    ...
                    console.error('Import Data/File错误:', error);
        ...
        if (event.startTime) {
            ...
            if (event.endTime) {
                ...
                if (startTime.toDateString() === endTime.toDateString()) {
                    ...
                    timeText += ` - ${endDateStr} ${endTimeStr}`;
                }
            }
        }
    ...
    handleExport() {
        ...
        if (!startDate || !endDate) {
            this.showNotification('Please set export date range');
            return;
        }
        ...
        const filter = {
            startDate,
            endDate: new Date(endDate + 'T23:59:59').toISOString() // End date set to the last moment of the day
        };
        ...
        if (!data) {
            this.showNotification('没有符合条件的Things');
            return;
        }
     ...
     * @param {Number} duration 持续time（毫秒）
     */
    ...
    initTheme() {
        ...
        if (theme === 'system') {
            ...
            // 启动定时检测 - 每5秒检查一times系统主题变化
            // 这是为了在媒体查询Things不可靠的情况下的备选方案
        ...
        try {
            ...
            this._darkModeChangeHandler = (e) => {
                ...
                if (currentSettings.theme === 'system') {
                    console.log('系统主题变化 (Things):', e.matches ? '深色' : '浅色');
            ...
            console.error('Set系统主题检测失败', error);
        }
    },
    ...
    /**
     * Set主题
     * @param {String} themeSetting 主题Set (light/dark/system/auto)
     */
    setTheme(themeSetting) {
        console.log('Set主题:', themeSetting);
    ...
    applyTheme(theme) {
        ...
        // SetHTML元素的data属性，便于CSSSelect器使用
        document.documentElement.setAttribute('data-theme', theme);
        
        // 触发自定义Things，允许其他组件响应主题变化
    ...
    switchView(viewId) {
        ...
        // 检查是否在Focus计时medium尝试切换到Downtime
        ...
        if (relaxView) {
            // 如果当前是计时状态，隐藏Downtime视图
            if (isTimerActive) {
                relaxView.classList.remove('active');
                relaxView.style.display = 'none';
            } else if (viewId === 'relax') {
                // 非Focus下，且切换到Downtime视图时，显示Downtime
                relaxView.style.display = '';
        ...
        this.elements.viewSections.forEach(section => {
            ...
            if (sectionId === 'focus-mode') {
                if (isTimerActive && viewId !== 'focus') {
                    // Focus正在计时但用户切换到其他视图 - 保持Focus在后台运行
                    section.classList.remove('active');
                    
                    // 不Setdisplay:none，而是把它移到视图之外但仍然保持活跃
    ...
    /**
     * 切换New视图Label页
     * @param {String} tabId Label页ID (traditional/ai/import)
     */
    ...
    backupToLocal() {
        try {
            ...
            this.showNotification(`已成功创建备份 (${backupMeta.eventCount}个Things, ${backupMeta.projectCount}个Item)`);
        } catch (error) {
    ...
    restoreFromLocal() {
        ...
        backups.forEach(backup => {
            ...
            backupItem.innerHTML = `
                <div class="backup-meta">
                    ...
                    <span class="backup-description">${backup.description || 'Empty描述'}</span>
                    <span class="backup-size">${size}</span>
                </div>
                <div class="backup-stats">
                    <span>Things: ${backup.eventCount || '未知'}</span>
                    <span>Item: ${backup.projectCount || '未知'}</span>
                </div>
        ...
        // 显示备份Select对话框
        this.showModal('Select要恢复的备份', backupsList);
    },
    ...
    confirmRestore(backup) {
        if (confirm(`您确定要恢复此备份吗？\n备份time: ${new Date(backup.createdAt).toLocaleString('zh-CN')}\n此操作将覆盖当前所有Content。`)) {
            // 执行恢复
    ...
    showBackupManager() {
            ...
            backups.forEach(backup => {
                ...
                backupItem.innerHTML = `
                    ...
                    <div class="backup-actions">
                        ...
                        <button class="delete-backup-btn">Delet</button>
                    </div>
                `;
                ...
                deleteBtn.addEventListener('click', () => {
                    ...
                    deleteBtn.textContent = 'Deletmedium...';
                    
                    if (confirm(`确定要Delet此备份吗？\n备份time: ${createdAt}`)) {
                        StorageManager.deleteBackup(backup.id);
                        this.showBackupManager(); // 刷新管理器
                        this.showNotification('备份已Delet');
                    } else {
                        // 用户点击取消，恢复按钮状态
        ...
        if (clearAllBackupsBtn) {
            clearAllBackupsBtn.addEventListener('click', () => {
                ...
                clearAllBackupsBtn.textContent = '清除medium...';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (userNickname) {
        ...
        // 确保立即显示积分（再times调用确保即使初始化过程medium有延迟也能正确显示）
        setTimeout(() => {
            UIManager.updateHeaderPoints();
        }, 100);
        
        // 确保Focus和Downtime视图状态一致
        setTimeout(() => {
            if (window.FocusManager) {
                ...
                // 为Downtime按钮重新绑定点击Things
                const relaxNavBtn = document.getElementById('nav-relax');
                if (relaxNavBtn) {
                    // 清除旧的Things监听器（防止重复绑定）
                    ...
                    newBtn.addEventListener('click', () => {
                        ...
                        if (isTimerActive) {
                            UIManager.showNotification('计时进行medium，Empty法进入Downtime');
                        } else {
...
function initAvatarUpload() {
    ...
    avatarInput.addEventListener('change', function(e) {
        ...
        // 检查文件类型
        if (!file.type.match('image.*')) {
            showToast('请Select图片文件（JPG、PNG格式）', 'error');
            return;
        }
