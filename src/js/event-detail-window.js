...
class EventDetailWindow {
    ...
    createWindow() {
        ...
        this.window.innerHTML = `
            ...
            <div class="event-detail-content" id="detail-content">
                ...
                <!-- 适宜事件推荐 -->
                <div class="detail-section" id="suitable-events-section">
                    <div class="detail-section-title">
                        <i class="fas fa-lightbulb"></i>
                        适宜事件
                    </div>
                    <div class="detail-section-content suitable-events-container" id="detail-suitable-events">
                        <div class="suitable-events-scroll" id="suitable-events-scroll">
                            <span class="empty">正在加载推荐...</span>
                        </div>
                    </div>
                </div>
    ...
    bindEvents() {
        ...
        // 为适宜事件区域添加专门的触屏滑动适配
        this.enableSuitableEventsTouchScroll();
    }
     ...
     * 为适宜事件区域添加专门的触屏滑动适配
     */
    enableSuitableEventsTouchScroll() {
        ...
        const createScrollIndicator = () => {
            ...
            scrollIndicator.className = 'suitable-events-scroll-indicator';
            scrollIndicator.style.cssText = `
                ...
                z-index: 10;
            `;
            suitableEventsContainer.appendChild(scrollIndicator);
        };
        ...
        const updateScrollIndicator = () => {
            ...
            const scrollHeight = suitableEventsScroll.scrollHeight;
            const clientHeight = suitableEventsScroll.clientHeight;
            const scrollTop = suitableEventsScroll.scrollTop;
            
            if (scrollHeight > clientHeight) {
                const scrollRatio = scrollTop / (scrollHeight - clientHeight);
                const indicatorHeight = Math.max(20, (clientHeight / scrollHeight) * 40);
                const indicatorTop = scrollRatio * (40 - indicatorHeight);
                
                scrollIndicator.style.height = `${indicatorHeight}px`;
                scrollIndicator.style.top = `${50 + indicatorTop}%`;
        ...
        // 触摸开始
        suitableEventsContainer.addEventListener('touchstart', (e) => {
            if (e.touches.length !== 1) return;
            
            startY = e.touches[0].clientY;
            startScrollTop = suitableEventsScroll.scrollTop;
            isScrolling = false;
            touchStartTime = Date.now();
            
            // 添加触摸状态类
            suitableEventsContainer.classList.add('touch-scrolling');
        ...
        // 触摸移动
        suitableEventsContainer.addEventListener('touchmove', (e) => {
            if (e.touches.length !== 1) return;
            ...
            // 如果内容可以滚动，则处理滚动
            if (suitableEventsScroll.scrollHeight > suitableEventsScroll.clientHeight) {
                // 计算新的滚动位置
                const newScrollTop = startScrollTop + deltaY;
                
                // 限制滚动范围
                const maxScrollTop = suitableEventsScroll.scrollHeight - suitableEventsScroll.clientHeight;
                const clampedScrollTop = Math.max(0, Math.min(newScrollTop, maxScrollTop));
                
                // 应用滚动
                suitableEventsScroll.scrollTop = clampedScrollTop;
        ...
        // 触摸结束
        suitableEventsContainer.addEventListener('touchend', (e) => {
            if (isScrolling) {
                // 添加惯性滚动效果
                ...
                if (deltaTime > 0 && Math.abs(deltaY) > 10) {
                    ...
                    // 应用惯性滚动
                    const currentScrollTop = suitableEventsScroll.scrollTop;
                    const targetScrollTop = currentScrollTop + momentum;
                    const maxScrollTop = suitableEventsScroll.scrollHeight - suitableEventsScroll.clientHeight;
                    const finalScrollTop = Math.max(0, Math.min(targetScrollTop, maxScrollTop));
                    
                    // 平滑滚动到目标位置
                    suitableEventsScroll.scrollTo({
                        top: finalScrollTop,
                        behavior: 'smooth'
                    });
                    ...
                    const checkScrollComplete = () => {
                        updateScrollIndicator();
                        if (Math.abs(suitableEventsScroll.scrollTop - finalScrollTop) > 1) {
            ...
            // 移除触摸状态类
            suitableEventsContainer.classList.remove('touch-scrolling');
            isScrolling = false;
        ...
        // 触摸取消
        suitableEventsContainer.addEventListener('touchcancel', () => {
            suitableEventsContainer.classList.remove('touch-scrolling');
            isScrolling = false;
        ...
        // 监听滚动事件更新指示器
        suitableEventsScroll.addEventListener('scroll', updateScrollIndicator);
    }
    ...
    switchToEditMode(eventId) {
        ...
        editBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> 切换到编辑模式...';
        ...
        setTimeout(() => {
            ...
            setTimeout(() => {
                ...
                // 显示友好的提示信息
                if (window.UIManager && window.UIManager.showNotification) {
                    window.UIManager.showNotification('已切换到编辑模式，您可以修改事件信息');
                }
