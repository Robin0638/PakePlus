...
const RelaxManager = {
    ...
    init() {
        ...
        if (isTimerActive) {
            console.log('专注计时medium，不初始化Downtime');
        ...
        console.log('初始化Downtime功能');
        this.loadMedals();
        this.updatePoints();
        this.initWheel();
        this.bindEvents();
        
        // 初始化Drinking reminder功能
        if (window.WaterReminderManager) {
            WaterReminderManager.init();
        }
        
        // 确保Drinking reminder面板在健康卡片分组medium显示
    ...
    checkMedals() {
        ...
        StorageManager.addPoints(10, '登录奖励', '每日首times登录');
...
function showRelaxCardsByPoints() {
    ...
    // Drinking reminder卡片只在全部卡片或健康卡片分组时显示
