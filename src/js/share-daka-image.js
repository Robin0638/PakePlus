// Check in卡片图片Share功能
// 依赖：html2canvas（需在index.htmlmedium引入）
(function(){
    function showShareDakaImageModal(dakaData) {
        ...
        temp.innerHTML = `
            ...
            <div style="margin-bottom:10px;font-size:13px;color:#888;">Label：${(dakaData.tags||[]).map(tag=>`<span style='background:#e3f0ff;color:#4285f4;padding:2px 8px;border-radius:4px;margin-right:6px;font-size:12px;'>${escapeHtml(tag)}</span>`).join('')}</div>
            <div style='margin-bottom:10px;font-size:13px;color:#888;'>Check in记录：</div>
            ...
            <div style='margin-top:18px;text-align:center;font-size:12px;color:#bbb;'>Content来自有数规划（电脑版）</div>
        `;
    ...
    function showImagePreviewModal(imgUrl) {
        ...
        overlay.innerHTML = `
            <div class="share-daka-image-popup">
                ...
                <div class="share-daka-image-actions">
                    <button class="share-daka-image-btn" id="daka-img-download"><i class="fas fa-download"></i> Download images</button>
                </div>
            </div>
        `;
        ...
        overlay.querySelector('#daka-img-share').onclick = async () => {
            if (navigator.canShare && navigator.canShare({ files: [] })) {
                ...
                try {
                    await navigator.share({ files: [file], title: 'Check inShare', text: 'Share我的Check in' });
                } catch {}
            } else {
                alert('当前浏览器不支持原生图片Share，可手动下载后Share');
            }
        };
    }
    ...
    window.showShareNoteImageModal = function(noteData) {
        ...
        temp.innerHTML = `
            ...
            <div style=\"margin-bottom:10px;font-size:13px;color:#888;\">Label：${(noteData.tags||[]).map(tag=>`<span style='background:#e3f0ff;color:#4285f4;padding:2px 8px;border-radius:4px;margin-right:6px;font-size:12px;'>${escapeHtml(tag)}</span>`).join('')}</div>
            <div style='margin-top:18px;text-align:center;font-size:12px;color:#bbb;'>Content来自有数规划（电脑版）</div>
        `;
        document.body.appendChild(temp);
        window.html2canvas(temp, {backgroundColor: null, useCORS: true, scale: 2}).then(canvas => {
    ...
    window.showShareCountdownImageModal = function(countdownData) {
        ...
        temp.innerHTML = `
            ...
            ${countdownData.notes?`<div style=\"font-size:14px;color:#888;margin-bottom:8px;word-break:break-all;\">Remark：${escapeHtml(countdownData.notes)}</div>`:''}
