 ...
 * @param {string} href 可选，分享链接
 */
function systemShareText(text, href) {
    if (window.plus && plus.share && plus.share.sendWithSystem) {
        var msg = { content: text };
        if (href) msg.href = href;
...
/**
 * 系统级分享图片/文件（终极兼容写法：先 resolve 到绝对路径再分享）
 * @param {string} filePath 本地文件路径
 */
function systemShareFile(filePath) {
    if (window.plus && plus.share && plus.share.sendWithSystem) {
        plus.io.resolveLocalFileSystemURL(filePath, function(entry) {
            var realPath = entry.toLocalURL();
            if (!realPath.startsWith('file://')) {
                realPath = 'file://' + realPath;
            }
            alert('即将分享文件绝对路径：' + realPath);
            var msg = { pictures: [realPath], files: [realPath] };
            plus.share.sendWithSystem(msg, function() {
                plus.nativeUI.toast('分享成功');
            }, function(e) {
                alert('分享失败! 路径: ' + realPath + '\n错误: ' + JSON.stringify(e));
            });
        }, function(e) {
            alert('路径解析失败: ' + filePath + '\n错误: ' + JSON.stringify(e));
        });
    } else {
        alert('请在App内使用系统分享');
    }
...
/**
 * 通用外部链接打开函数
 * 优先使用uni-app的plus.runtime.openURL API，降级到window.open
 * @param {string} url - 要打开的URL
 * @param {string} fallbackMessage - 降级时的提示消息
 */
function openExternalUrl(url, fallbackMessage = '正在打开链接...') {
    // 优先使用uni-app的plus.runtime.openURL API
    if (window.plus && plus.runtime && plus.runtime.openURL) {
        plus.runtime.openURL(url);
        if (typeof showCopyTip === 'function') {
            showCopyTip(fallbackMessage);
        }
    } else {
        // 降级到window.open
        const newWindow = window.open(url, '_blank', 'noopener,noreferrer');
        
        // 如果window.open被阻止，提示用户手动打开
        if (!newWindow || newWindow.closed || typeof newWindow.closed == 'undefined') {
            // 创建提示弹窗，让用户手动复制链接
            const manualOpen = confirm('浏览器阻止了自动跳转。\n\n请点击"确定"复制链接到剪贴板，然后手动在浏览器中打开。');
            if (manualOpen) {
                // 复制链接到剪贴板
                if (navigator.clipboard) {
                    navigator.clipboard.writeText(url).then(() => {
                        if (typeof showCopyTip === 'function') {
                            showCopyTip('链接已复制到剪贴板！\n请在浏览器中粘贴并访问。');
                        }
                    }).catch(() => {
                        if (typeof showCopyTip === 'function') {
                            showCopyTip('复制失败，请手动复制链接：\n' + url);
                        }
                    });
                } else {
                    // 兼容旧浏览器
