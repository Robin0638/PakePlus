    ...
    events.forEach(event => {
        ...
        eventItem.innerHTML = `
            ...
            <div class="event-actions">
                ...
                <button class="btn btn-small ${disableClass}" onclick="deleteEvent(${event.id})" style="background: #e74c3c;" ${disableBtn}>删除</button>
            </div>
        `;
...
function pauseTimer() {
    ...
    try {
        ...
        sendFocusMessage('focusPause', {
            eventName: currentEvent ? currentEvent.name : '已暂停',
            timeLeft: timeLeft
        });
...
function completePomodoro() {
    ...
    try {
        ...
        sendFocusMessage('focusComplete', {
            eventName: currentEvent ? currentEvent.name : '专注完成',
            timeLeft: 0
        });
...
function importData(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            // 询问用户合并还是覆盖
            const keepOld = confirm('是否保留原有内容？\n选择“确定”将合并数据，选择“取消”将覆盖原有数规划（电脑版）据。');
            if (keepOld) {
                // 合并数据
                // 合并 events（按 id 去重）
                const eventMap = {};
...
function checkDailyReset() {
    ...
    if (lastReset !== today) {
        ...
        // 如果昨天有数规划（电脑版）据且还没有保存到dailyStats中
