...
const FocusStatusManager = {
    ...
    config: {
        ...
        maxEventNameLength: 15, // Name最大显示长度
    ...
    immediateStatusCheck() {
        ...
        // 3. 检查localStoragemedium的focusStats
        this.checkFocusStats();
        
        // 4. 更新显示
        this.updateDisplay();
        
        console.log('立即检查Completed，当前状态:', this.currentStatus);
    },
            ...
            if (window.StorageManager) {
                ...
                if (data.focusTime && data.focusTime.history) {
                    ...
                    if (todayRecord && todayRecord.minutes > 0) {
                        // 如果有今日专注记录但没有正在进行的专注，显示Completed状态
        ...
        try {
            ...
            if (focusStats) {
                ...
                if (stats.isRunning === false && stats.timeLeft > 0) {
                    this.setStatus(this.STATUS.PAUSED, {
                        eventName: stats.currentEvent || 'Stopped',
                ...
                if (stats.isRunning && this.currentStatus === this.STATUS.IDLE) {
                    this.setStatus(this.STATUS.ACTIVE, {
                        eventName: stats.currentEvent || '专注medium',
                        timeLeft: stats.timeLeft || 0
                    });
                }
            }
    ...
    updateFromFocusStats(stats) {
        ...
        if (isRunning === false && timeLeft > 0) {
            this.setStatus(this.STATUS.PAUSED, {
                eventName: currentEvent || 'Stopped',
                timeLeft: timeLeft || 0
            });
        } else if (isRunning === true) {
            this.setStatus(this.STATUS.ACTIVE, {
                eventName: currentEvent || '专注medium',
                timeLeft: timeLeft || 0
            });
        } else if (completedPomodoros > 0 || totalFocusTime > 0) {
            if (this.currentStatus === this.STATUS.IDLE) {
                this.setStatus(this.STATUS.COMPLETED, {
                    eventName: currentEvent || '专注Completed',
                    timeLeft: 0
                });
            }
        } else {
    ...
    setStatus(status, data = {}) {
        // 允许同一Things不同状态切换（如active<->paused），但同一状态同一Content不Repeat渲染
    ...
    updateDisplay() {
        ...
        if (eventElement) {
            ...
            if (this.currentStatus === this.STATUS.PAUSED) {
                eventName = eventName === 'Stopped' ? eventName : `${eventName}（Stopped）`;
            } else if (this.currentStatus === this.STATUS.COMPLETED) {
                eventName = eventName === '专注Completed' || eventName === '今日专注' ? eventName : `${eventName}（Completed）`;
            }
    ...
    handleFocusStart(data) {
        this.setStatus(this.STATUS.ACTIVE, {
            eventName: data.eventName || '专注medium',
    ...
    handleFocusPause(data) {
        this.setStatus(this.STATUS.PAUSED, {
            eventName: data.eventName || 'Stopped',
            timeLeft: data.timeLeft || 0
        });
    },
    ...
    handleFocusComplete(data) {
        this.setStatus(this.STATUS.COMPLETED, {
            eventName: data.eventName || '专注Completed',
            timeLeft: 0
        });
