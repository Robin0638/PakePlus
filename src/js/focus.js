...
const FocusManager = {
        ...
        if (window.UIManager) {
            ...
            if (isTimerActive) {
                ...
                if (relaxNavBtn) {
                    ...
                    relaxNavBtn.title = '计时进行medium，Empty法进入Downtime';
                }
    ...
    loadTaskOptions() {
        ...
        if (!events || events.length === 0) {
            ...
            option.value = '';
            option.text = '没有可用的Things';
            option.disabled = true;
            this.elements.taskSelector.add(option);
            return;
        }
        
        // 按照开始time排序Things，最近的Things排在前面
        events.sort((a, b) => {
        ...
        // Empty论是否恢复了之前的Select，都更新任务专注time显示
    ...
    init() {
        ...
        if (savedState.active) {
            ...
            if (this.startTime) {
                ...
                // 如果剩余time小于等于0，重置专注（可能是之前未正确结束的会话）
                if (this.remainingTime <= 0) {
                    this.resetFocus();
                    this.setupInitialState();
                } else {
                    // SetCSS变量标记Focus计时已开始
        ...
        // 绑定Things - 必须在加载Set之后绑定，确保Things处理能访问到正确的Set
        this.bindEvents();
    ...
    setupInitialState() {
        ...
        // 初始化Downtime状态 - 初始化时默认不在计时状态，保持Downtime按钮可用
        const relaxNavBtn = document.getElementById('nav-relax');
        if (relaxNavBtn) {
            relaxNavBtn.classList.remove('disabled');
            relaxNavBtn.style.pointerEvents = 'auto';
    ...
    pauseFocus() {
        ...
        if (this.strictMode) {
            console.log('严格模式已启用，Empty法暂停');
            // 播放错误音效
            this.playSound('error');
            // 显示提示
            if (window.UIManager) {
                UIManager.showNotification('严格模式已启用，Empty法暂停计时');
            }
            return;
        }
        ...
        document.title = `${formattedTime} - Stopped | 有数规划（电脑版）`;
    },
    ...
    resumeFocus() {
        ...
        if (this.lastPauseTime) {
            ...
            console.log(`暂停time: ${pauseDuration}秒，总暂停time: ${this.pausedTime}秒`);
        }
        ...
        const seconds = this.remainingTime % 60;
        const formattedTime = `${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')}`;
        document.title = `${formattedTime} - 专注medium | 有数规划（电脑版）`;
    },
    ...
    resetFocus() {
        ...
        // 如果当前处于活动状态且已经开始计时，记录已经Completed的time
        if ((this.status === 'active' || this.status === 'paused') && this.startTime) {
            ...
            console.log(`总经过time: ${totalElapsedSeconds}秒，暂停time: ${this.pausedTime || 0}秒，实际专注: ${focusedSeconds}秒`);
        ...
        try {
            ...
            console.error('Purge专注状态失败', error);
        }
    },
    ...
    completeFocus() {
        ...
        console.log(`专注Completed - 总time: ${totalMinutes}min，暂停: ${pausedMinutes}min，实际专注: ${duration}min`);
        
        // 记录Completed并Update time统计
        this.logFocusComplete();
        
        // 更新专注统计
        this.updateFocusStats(duration);
        
        // 更新专注time记录和获取积分信息（仅任务相关的逻辑）
        let pointsInfo = null;
        if (this.elements.taskSelector) {
        ...
        // 显示Completed通知
        if (pointsInfo) {
            let notificationText = `专注Completed！\n`;
            notificationText += `本times专注：${duration}min\n`;
            notificationText += `获得积分：${pointsInfo.totalPoints}分\n`;
            notificationText += `（基础${pointsInfo.basePoints}分，连续奖励${pointsInfo.streakBonus}分）\n`;
            notificationText += `今日累计：${pointsInfo.todayDuration}min，${pointsInfo.todayPoints}分`;
            
            UIManager.showNotification(notificationText);
        } else {
            UIManager.showNotification('专注Completed！');
        }
    ...
    /**
     * 更新专注统计
     * @param {number} duration Time（min）
     */
    updateFocusStats(duration) {
        ...
        if (typeof duration !== 'number' || isNaN(duration)) {
            console.error('Empty效的Time:', duration);
            return;
        }
    ...
    updateFocusTimeDisplay() {
        ...
        this.elements.todayFocusTime.textContent = `${todayMinutes}min`;
        
        // 更新总专注time
        const totalMinutes = Math.floor((data.totalFocusTime || 0) / 60);
        this.elements.totalFocusTime.textContent = `${totalMinutes}min`;
    },
    ...
    updateUIForActive() {
        ...
        if (this.elements.pauseButton) {
            ...
            if (this.strictMode) {
                ...
                this.elements.pauseButton.title = '严格模式已启用，Empty法暂停';
            } else {
        ...
        const seconds = this.remainingTime % 60;
        const formattedTime = `${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')}`;
        document.title = `${formattedTime} - 专注进行medium`;
     ...
     * @param {String} message 通知Content
     */
    ...
    updateTaskFocusTimeDisplay() {
        ...
        // 如果没有Select任务，显示默认值
        if (!selectedTask) {
            if (taskFocusTimeElement) taskFocusTimeElement.textContent = '0min';
        ...
        // 如果没有数规划（电脑版）据或没有该任务的Stats，显示默认值
        ...
        // 更新任务专注time显示（如果有对应元素）
        if (taskFocusTimeElement) {
            const taskMinutes = Math.floor(taskStats.totalFocusTime / 60);
            taskFocusTimeElement.textContent = `${taskMinutes}min`;
        }
        
        // 更新任务专注times数显示（如果有对应元素）
