...
class DecibelMeter {
    ...
    init() {
        this.bindEvents();
        this.updateStatus('Click to start measuring');
    }
    ...
    async startMeasurement() {
        try {
            ...
            console.error('Empty法访问麦克风:', error);
            this.updateStatus('Empty法访问麦克风，请检查权限Set');
    ...
    stopMeasurement() {
        ...
        this.updateExtremes('--', '--'); // 停止时重置最high/最低值
    }

    /**
     * 测量循环（更平稳：加大平滑+限制刷新频率+只显示整数+最high最低值）
     */
    measureLoop() {
    ...
    getDecibelDescription(decibel) {
        if (decibel < 20) {
            return '很静，几乎感觉不到（Empty危害）';
        } else if (decibel < 40) {
            return '安静，犹如轻声说话（Empty危害）';
        } else if (decibel < 60) {
            return '一般普通室内谈话（Empty危害）';
        } else if (decibel < 70) {
    ...
    updateExtremes(min, max) {
        ...
        if (maxEl) maxEl.textContent = `最high：${max} dB`;
    }
}
