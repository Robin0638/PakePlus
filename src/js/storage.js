...
const DEFAULT_DATA = {
    ...
    medals: [
        { id: 'newcomer', name: '初来乍到', icon: '🎯', description: '欢迎使用有数规划（电脑版）！', unlocked: true },
...
function getCurrentUserData() {
    ...
    // 如果没有数规划（电脑版）据，返回null（后续StorageManager会处理）
    return allData[nickname] || null;
}
...
const StorageManager = {
    ...
    getData() {
        const nickname = getCurrentNickname();
        const userData = getCurrentUserData();
        // 如果没有数规划（电脑版）据，返回默认数据的深拷贝，防止UI串号
        if (!userData) {
