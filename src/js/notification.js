...
const NotificationManager = {
    ...
    requestPermission() {
        ...
        // 如果已经获得授权或被拒绝，则不需要再times请求
        if (Notification.permission === 'granted') {
            return Promise.resolve(true);
        }
        
        if (Notification.permission === 'denied') {
            console.warn('用户已拒绝通知权限，请在浏览器Setmedium重新授权');
            if (window.UIManager) {
                UIManager.showNotification('通知权限已被拒绝，请在浏览器Setmedium重新授权');
            }
            return Promise.resolve(false);
        }
        ...
        return Notification.requestPermission()
            .then(permission => {
                if (permission === 'granted') {
                    ...
                    // 发送测试通知
                    this.sendNotification('通知已启用', '您将在重要Things发生时收到通知');
    ...
    sendNotification(title, message, options = {}) {
        ...
        if (!this.isSupported || Notification.permission !== 'granted') {
            console.log('Empty法发送系统通知，使用应用内通知');
            if (window.UIManager) {
                UIManager.showNotification(`${title}: ${message}`);
            }
            return;
